...
class Dictionary extends Controller
{
    ...
    {
    	$list=model('Dictionary')->getlist();    	
    	$this->assign('data',$list);
		$this->assign('title','单词列表');   	
        return $this->fetch();
    }
	public function add(){
	...
	public function save(){
		if(request()->isPost()){
			$post=input('post.');
			if(!validate('Dictionary')->scene('add')->check($post)){
				$this->error('数据有误，新增失败');
			}
			$res=model('Dictionary')->add($post);
			if($res>0){
				$this->success('新增成功',url('Dictionary/index'));
			}else{
				$this->error('新增失败');
			}
		}
